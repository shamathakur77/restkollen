...
# Static, human-written, factual one-sentence descriptions. No medical
...
<body>
<a class="skip-link" href="#cat-list"><span class="sv">Hoppa till innehåll</span><span class="en">Skip to content</span></a>
...
<main class="wrap">
  <h1><span class="sv">{sv}</span><span class="en">{en}</span></h1>
  <p class="cat-desc"><span class="sv">{desc_sv}</span><span class="en">{desc_en}</span></p>
  <p class="count-line" id="cat-count"></p>
  ...
  <ul class="shortage-list" id="cat-list"></ul>
</main>
...
def main() -> None:
    out_dir = Path(__file__).resolve().parent.parent / "public" / "kategori"
    out_dir.mkdir(parents=True, exist_ok=True)
    for cat in CATEGORIES:
        desc_sv, desc_en = DESCRIPTIONS[cat["slug"]]
        prefixes_js = "[" + ", ".join(f'"{p}"' for p in cat["prefixes"]) + "]"
        html = TEMPLATE.format(
            sv=cat["sv"], en=cat["en"], slug=cat["slug"],
            desc_sv=desc_sv, desc_en=desc_en, prefixes_js=prefixes_js,
        )
        (out_dir / f"{cat['slug']}.html").write_text(html, encoding="utf-8")
        print(f"wrote kategori/{cat['slug']}.html")
